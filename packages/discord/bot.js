...
client.on('ready', () => {
  console.log("Bot artık RAGEMP CDN ve GTACity database'ini izliyor.")
  timer = setInterval(() => {
    scrapOnline();
  ...
  if (message.member.roles.cache.some(role => role.name === 'Server Staff') && message.channelId == config.channelId) {
    ...
    if (message.content.startsWith(`${prefix}vinfo`)) {
      ...
      vehicles.findAll({ where: { id: args } }).then((veh) => {
        if (veh.length > 0) {
          message.channel.send(`***Araç bilgisi:***` + "```" + JSON.stringify(veh[0]) + '```')
          return
        }
        message.channel.send(`Bu SQLID ile araç bulunamadı.`)
      }).catch((err) => { console.log(err) })
    }

    if (message.content.startsWith(`${prefix}getnamebyunix`)) {
      if (!args[0] || !args[1]) return message.channel.send(`Kullanım: !getnamebyunix **[Oyun içi ID] [UNIX]**`)
  ...
  try {
    ...
    if (result) {
      ...
      if (server !== undefined) {
        ...
        const statuses = [`${online == 0 ? online + ' player' : online + ' players'}`, `${charactersLen} registered characters`, `${vehiclesLen} total vehicles`, `${accountsLen} accounts registered with the server`, `paramountroleplay.com`]
