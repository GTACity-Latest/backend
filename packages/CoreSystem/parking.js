...
mp.events.add({
    "playerEnterColshape": (player, shape) => {
        if (shape.getVariable('parkingArea') && player.getVariable('loggedIn')) {
            if(!player.vehicle) { player.call('requestBrowser', [`gui.notify.showNotification("'Y' tuşuna basarak etkileşime geçin.", true, false, false, 'fa-solid fa-circle-info')`]) }
        ...
        if(shape.getVariable('parkingLot') && player.getVariable('loggedIn')) {
            ...
            if(player.vehicle) {
                player.call('requestBrowser', [`gui.notify.showNotification("/park kullanarak aracınızı garaja teslim edin.", true, false, false, 'fa-solid fa-circle-info')`])
            }
        }
    },
    ...
    'getParkedVehicles': async (player) => {
        ...
        vehicles.findAll({ where: { OwnerId: player.characterId, spawned: 0, parked: 1, insurance: 0, parkedArea: player.isInParking} }).then((getPlayerOwnedVehs) => {
            if (getPlayerOwnedVehs.length == 0) return mp.chat.err(player, `Park edilmiş bir aracınız yok.`)
            ...
            getPlayerOwnedVehs.forEach((veh) => {
                player.call('requestBrowser', [`appSys.commit('updateLists', {
                    menuName: 'Park Görevlisi',
                    menuSub: 'Şu an ${getPlayerOwnedVehs.length} aracınız parkta.',
                    tableOne: 'Name',
    ...
    'unparkVehicle': async (player, id) => {
        ...
        if(!vehInParking) {
            ...
            vehicles.findAll({ benchmark: true, logging: (sql, timingMs) => player.getVariable('devdebug') ? mp.chat.server(player, `[Execution time: ${timingMs}ms]`) : '', where: { id: parseInt(id) } }).then((findVeh) => {
                ...
                mp.chat.success(player, `Başarıyla aracınızı garajdan aldınız ${vehicle.numberPlate}`)
                vehicle.locked = true;
        ...
        else if(vehInParking) { return mp.chat.info(player, `Otoparkı kapatan bir araç var. Aracı hareket ettirmek için yardım alın.`) }
    },
});

mp.events.addCommand({
    'park': async (player, arg) => {
        if (arg != undefined) return mp.chat.info(player, `Kullanım: /park`)
            ...
            db.vehicles.findAll({ benchmark: true, logging: (sql, timingMs) => player.getVariable('devdebug') ? mp.chat.server(player, `[Execution time: ${timingMs}ms]`) : '', where: { id: player.vehicle.getVariable('sqlID') } }).then((veh) => {
                if (veh.length > 0 && veh[0].OwnerId == player.characterId) {
                    ...
                    db.vehicles.update({
                        ...
                        spawned: 0
                    }, { where: { id: player.vehicle.getVariable('sqlID') } }).then(() => { mp.chat.success(player, `Aracını bu plaka ile park ettin: ${player.vehicle.numberPlate}`), player.vehicle.destroy() })
                }
                else { mp.chat.err(player, `Bu araca sahip değilsin.`) }
            })
            return;
        }
    }
