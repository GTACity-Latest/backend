...
mp.events.add({
    ...
    'getVFromInsurance': async(player, id) => {
        ...
        if(vehInParking) { return mp.chat.info(player, `Sigorta park yerinin önünü kapatan bir araç var. Aracı hareket ettirmek için yardım alın.`)}


        const { vehicles } = require('../models')
        vehicles.findAll({ benchmark: true, logging: (sql, timingMs) => player.getVariable('devdebug') ? mp.chat.server(player, `[Yürütme süresi: ${timingMs}ms]`) : '', where: {id: parseInt(id)} }).then((findVeh) => {
            ...
            mp.chat.success(player, `Aracınızı geri aldınız ${vehicle.numberPlate} araç sigortasından.`)
            vehicle.locked = true;
